...
const CourseRegistration = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 1. Save student registration info
      ...
      navigate("/success", {
        state: {
          application: res.data.application, // Make sure your backend returns this
          courses: applyCourses.map(course => {
            const institute = institutes.find(i => i._id === course.institute_id);
            return {
              ...course,
              reference_number: res.data.application?.reference_number, // assuming it’s returned
